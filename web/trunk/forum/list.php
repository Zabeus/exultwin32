...
////////////////////////////////////////////////////////////////////////////////
  ...
  if ($ForumMultiLevel==2) {
    ...
    if($thread==0 || $action==0){
      ...
      if($$phcollapse==0){
        $SQL = "Select thread, threadflags from $ForumTableName where approved='Y' and thread < $thread and thread > $cutoff_thread order by thread desc".$limit;
      }
      else{
        $SQL = "Select thread, COUNT(id) AS tcount, MAX(datestamp) AS latest, MAX(id) AS maxid, threadflags FROM $ForumTableName WHERE approved='Y' AND thread < $thread AND thread > $cutoff_thread GROUP BY thread ORDER BY thread DESC".$limit;
      }
    }
